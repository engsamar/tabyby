        ...
        <div id="name-search" hidden>
            <form action="/reserv/searchByName" method="POST">
                ...
                <div class="form-group @if($errors->has('name')) has-error @endif">
                    ...
                    <input list="searchResult" type="text" id="name-field" name="name" class="form-control"
                           value="{{ old("name") }}"/>
                                    ...
                                    @if($userRole == 0)
                                        ...
                                        <td class="text-right">
                                            <a class="btn btn-xs btn-primary"
                                               href='/all_reservation/{{$reservation->user_id}}'><i
                                                        class="glyphicon glyphicon-eye-open"></i> View History</a>
                                        </td>
                                    @else
                                        ...
                                        <td class="text-right">
                                            <form action="{{ route('reservations.destroy', $reservation->id) }}"
                                                  ...
                                                  onsubmit="if(confirm('Cancel Reservation ? Are you sure?')) { return true } else {return false };">
                                                <input type="hidden" name="_method" value="DELETE">
                                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                <a class="btn btn-xs btn-warning btn-group" role="group"
                                                   href="{{ route('reservations.edit', $reservation->id) }}"><i
                                                            class="glyphicon glyphicon-edit"></i> Postpone</a>
                                                <button type="submit" class="btn btn-xs btn-danger">Cancel Reservation
